...
console.log("[Musica] app.js v2.12 (chapter sort + scrollable chapters)");
...
let lastPct = 0;
let chapterSortDir = "asc"; // ★ hướng sắp xếp chapter: asc (1→n) / desc (n→1)
...
// ★ Render các thẻ arc — chip chương là NÚT BẤM, click = phát luôn
function renderArcs() {
  ...
  groups.forEach((group) => {
    ...
    card.innerHTML = `
      <div class="arc-cover">${ungrouped ? "…" : "♢"}</div>
      <div class="arc-info">
    ...
    arcCards.push({ card, arc, chapters: chs, ungrouped });
  });
}

// ★ Bấm chip → phát bài đầu của chương + cuộn tới thẻ chương
...
function renderChapters() {
  ...
  // Sắp xếp theo chapter_number: asc = 1 → nhiều, desc = nhiều → 1
  ...
  sorted.forEach(chapter => {
    // Giữ index GỐC trong mảng chapters để playTrack hoạt động đúng
...
document.querySelector("#clearPlayer").onclick = stopEverything;
document.querySelector("#exitPlayer").onclick = stopEverything;
...
/* =========================================================
   APP.JS — DÁN THÊM VÀO CUỐI FILE (khối mới, không xóa gì)
   1) Nút vuông giữa: Play/Pause thay vì thoát bài
   2) Volume slider + nút mute
========================================================= */
...
// Thay handler cũ của nút giữa (■ thoát bài → ▶ play/pause)
const ppBtn = document.querySelector("#clearPlayer");
if (ppBtn) ppBtn.onclick = togglePlayPause;

// Nút thoát (✕) vẫn dừng hẳn bài, nhưng reset icon về ▶
const exitBtn = document.querySelector("#exitPlayer");
if (exitBtn) exitBtn.onclick = () => { stopEverything(); setPlayPauseIcon(false); };

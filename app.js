...
function createYoutubePlayer() {
  ...
  youtubePlayer = new YT.Player("youtubePlayer", {
    ...
    events: {
      onStateChange: (event) => {
        // Đang phát → bắt đầu cập nhật tiến trình
        ...
        // Video phát hết → tự chuyển (theo công tắc Auto)
...
function resetProgress() {
  ...
  const dur = document.querySelector("#progDuration");
...
function updateProgress(current, duration) {
  ...
  if (durEl) {
    if (duration === Infinity) {
      ...
      durEl.textContent = total > 0 ? fmtTime(total) : "0:00";
    }
  }
...
async function playTrack(chapterIndex, trackIndex) {
  ...
  resetProgress(); // bài mới → thanh tiến trình về 0
...
function nextTrackAuto() {
  ...
  if (next < chapter.tracks.length) {
    ...
    // Hết chapter rồi → chuyển sang chapter kế tiếp
    moveToNextChapter();
  }
}

// ★ Nhảy sang chapter kế tiếp (lấy bài đầu tiên của chapter đó)
//   Chapter trống thì tự bỏ qua; hết chapter cuối thì quay vòng về chapter đầu.
function moveToNextChapter() {
  ...
  // Không chapter nào có bài → không làm gì cả
}
...
// --- Thanh tiến trình: kéo/click để tua ---

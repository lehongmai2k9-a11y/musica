...
console.log("[Musica] app.js v2.16 (search ranking fix)");
...
function applySearch() {
  ...
  // ============ ARCS — xếp theo độ chuẩn của tên, KHÔNG ẩn arc nào ============
  // Điểm: trùng 100% (100) > bắt đầu bằng (80) > chứa (60) > từ bắt đầu bằng (30) > không khớp (0)
  function arcScore(name, q) {
    if (!q) return 0;
    const n = (name || "").toLowerCase();
    if (!n.includes(q)) {
      if (n.split(/\s+/).some(w => w.startsWith(q))) return 30;
      return 0; // không khớp → giữ nguyên, chỉ tụt xuống dưới các arc khớp
    }
    if (n === q) return 100;
  ...
  const rankedArcs = arcCards
    .map(entry => ({ entry, score: arcScore(entry.arc.name, arcsQ) }))
    .sort((a, b) => b.score - a.score); // khớp càng chuẩn càng lên đầu, còn lại giữ thứ tự cũ

  const wrap = document.querySelector("#arcsList");
  if (wrap) rankedArcs.forEach(({ entry }) => wrap.appendChild(entry.card));

  // Tô viền các arc khớp để dễ nhìn
  rankedArcs.forEach(({ entry, score }) => {
    entry.card.classList.toggle("search-hit", score > 0);
  });

  const arcMatches = rankedArcs.filter(r => r.score > 0).length;
  const arcEmpty = document.querySelector("#searchEmpty");
  if (arcEmpty) {
    arcEmpty.hidden = !(arcsQ && arcMatches === 0);
    arcEmpty.textContent = `Không tìm thấy arc nào khớp "${arcsQ}". Đang hiển thị toàn bộ danh sách.`;
  }

  // ============ CHAPTERS — gõ SỐ tìm theo số, gõ CHỮ tìm theo tên, KHÔNG ẩn chap nào ============
  const isNumber = /^\d+$/.test(chQ);

  const rankedCh = chapterCards
    .map(({ card, chapter }) => {
      let score = 0;
      if (chQ && isNumber) {
        // Nhập số → chương trùng đúng số lên đầu, sau đó số bắt đầu bằng
        const numStr = String(chapter.chapter_number);
        if (numStr === chQ || numStr.padStart(2, "0") === chQ) score = 100;
        else if (numStr.startsWith(chQ)) score = 50;
      } else if (chQ) {
        // Nhập chữ → tên trùng chuẩn > bắt đầu bằng > chứa
        const t = (chapter.title || "").toLowerCase();
        if (t === chQ) score = 90;
        else if (t.startsWith(chQ)) score = 70;
        else if (t.includes(chQ)) score = 50;
      }
      return { card, score };
    })
    .sort((a, b) => b.score - a.score);

  const chWrap = document.querySelector("#chaptersList");
  if (chWrap) rankedCh.forEach(({ card }) => chWrap.appendChild(card));

  // Tô viền các chap khớp
  rankedCh.forEach(({ card, score }) => {
    card.classList.toggle("search-hit", score > 0);
  });

  const chMatches = rankedCh.filter(r => r.score > 0).length;
  const chEmpty = document.querySelector("#chapterSearchEmpty");
  if (chEmpty) {
    chEmpty.hidden = !(chQ && chMatches === 0);
    chEmpty.textContent = `Không tìm thấy chapter nào khớp "${chQ}". Đang hiển thị toàn bộ danh sách.`;
  }
}
  ...
  if (next >= chapter.tracks.length) {
    if (arcPlayMode && direction > 0) {
      ...
      for (let i = 1; i <= groups.length; i++) {
        const gi = (currentArcIndex + i
/* =========================================================
   PHẦN 2 — nối tiếp ngay sau dòng cuối của Phần 1
   (Phần 1 kết thúc bằng: "const gi = (currentArcIndex + i")
   Dòng đầu tiên bên dưới là phần còn lại của câu lệnh đó.
========================================================= */
) % groups.length;
        const g = groups[gi];
...
applyVolume(100);
/* ════════════════════════════════════════════════════════
   NGHE NHẠC TRONG NỀN — giống SoundCloud
   Media Session API: hiển thị + điều khiển trên màn Đm
   khoá / nút tai nghe / thông báo, kể cả khi tab ẩn nền.
   ════════════════════════════════════════════════════════ */
(function setupMediaSession() {
  if (!("mediaSession" in navigator)) return;

  const ready = () => {
    const titleEl = document.querySelector("#nowTitle");
    const artistEl = document.querySelector("#nowArtist");
    if (!titleEl || !artistEl) return;

    // Cập nhật metadata mỗi khi đổi bài (theo dõi sự đổi của tên bài)
    const applyMeta = () => {
      const track = chapters[currentChapterIndex]?.tracks[currentTrackIndex];
      if (!track) return;
      let art = trackArtUrl(track) || "";
      if (!art.startsWith("http")) art = "";

      navigator.mediaSession.metadata = new MediaMetadata({
        title: track.title || "Musica",
        artist: track.artist || sourceType(track.source_type) || "Musica",
        album: chapters[currentChapterIndex]?.title || "Musica — Music Archive",
        artwork: art ? [{ src: art, sizes: "512x512" }] : [],
      });
    };

    const mo = new MutationObserver(applyMeta);
    mo.observe(titleEl, { childList: true, characterData: true, subtree: true });
    applyMeta();

    // Gắn điều khiển từ lock-screen / nông nghe
    try { navigator.mediaSession.setActionHandler("play",  () => { if (currentChapterIndex >= 0) togglePlayPause(); }); } catch (_) {}
    try { navigator.mediaSession.setActionHandler("pause", () => { if (currentChapterIndex >= 0) togglePlayPause(); }); } catch (_) {}
    try { navigator.mediaSession.setActionHandler("previoustrack", () => moveTrack(-1)); } catch (_) {}
    try { navigator.mediaSession.setActionHandler("nexttrack",     () => moveTrack(1)); } catch (_) {}
    try {
      navigator.mediaSession.setActionHandler("seekto", (d) => {
        if (d && d.seekTime != null) {
          if (isYoutubeActive()) {
            try { youtubePlayer.seekTo(d.seekTime, true); } catch (_) {}
          } else {
            const a = document.getElementById("audio");
            if (a && a.src) a.currentTime = d.seekTime;
          }
        }
      });
    } catch (_) {}

    // Đồng bộ trạng thái play/pause + thời gian tua
    const audio = document.getElementById("audio");
    if (audio) {
      audio.addEventListener("play",  () => { try { navigator.mediaSession.playbackState = "playing"; } catch (_) {} });
      audio.addEventListener("pause", () => { try { navigator.mediaSession.playbackState = "paused"; } catch (_) {} });
...
(function setupMediaSession() {
  ...
  const ready = () => {
    ...
    try {
      navigator.mediaSession.setActionHandler("seekto", (d) => {
        if (d && d.seekTime != null) {
          if (isYoutubeActive()) { try { youtubePlayer.seekTo(d.seekTime, true); } catch (_) {} }
          else { const a = document.getElementById("audio"); if (a && a.src) a.currentTime = d.seekTime; }
        }
      });
    } catch (_) {}
...
/* ════════════════════════════════════════════════════════
   NGHE NHẠC TRONG NỀN — giống SoundCloud (Media Session)
   Cho phép điều khiển play/pause, bài kế/trước, tua
   từ màn hình khóa / tai nghe / thông báo, kể cả khi
   tab bị thu nhỏ hoặc chuyển sang app khác.
   ════════════════════════════════════════════════════════ */
(function setupMediaSession() {
  if (!("mediaSession" in navigator)) return; // trình duyệt cũ thì bỏ qua

  const ready = () => {
    const titleEl = document.querySelector("#nowTitle");
    if (!titleEl) return;

    // Cập nhật tên bài / nghệ sĩ / ảnh lên màn hình khóa
    const applyMeta = () => {
      const track = chapters[currentChapterIndex]?.tracks[currentTrackIndex];
      if (!track) return;
      let art = trackArtUrl(track) || "";
    ...
    // Theo dõi mỗi lần đổi bài -> cập nhật tự động
    const mo = new MutationObserver(applyMeta);
    mo.observe(titleEl, { childList: true, characterData: true, subtree: true });
    applyMeta();

    // Gắn nút điều khiển từ lock screen / tai nghe
    try { navigator.mediaSession.setActionHandler("play", () => { if (currentChapterIndex >= 0) togglePlayPause(); }); } catch (_) {}
    try { navigator.mediaSession.setActionHandler("pause", () => { if (currentChapterIndex >= 0) togglePlayPause(); }); } catch (_) {}
    try { navigator.mediaSession.setActionHandler("previoustrack", () => moveTrack(-1)); } catch (_) {}
    try { navigator.mediaSession.setActionHandler("nexttrack", () => moveTrack(1)); } catch (_) {}
    ...
    // Đồng bộ trạng thái play/pause + vị trí tua
    const audio = document.getElementById("audio");
    if (audio) {
      audio.addEventListener("play", () => { try { navigator.mediaSession.playbackState = "playing"; } catch (_) {} });
      audio.addEventListener("pause", () => { try { navigator.mediaSession.playbackState = "paused"; } catch (_) {} });

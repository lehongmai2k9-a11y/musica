...
async function loadTracks(chapterId, container) {
  ...
  data.forEach(track => {
    ...
    // ★ Ảnh nhỏ của bài hát (nếu có ảnh riêng)
    let thumbHtml = "";
    if (track.cover_path) {
...
function openTrack(chapterId) {
  ...
  $("trackUrl").value = "";
  $("trackFile").value = "";
  $("trackCoverFile").value = "";   // ★ reset ảnh bài hát
...
async function saveTrack() {
  ...
  const coverFile = $("trackCoverFile").files?.[0]; // ★ ảnh bài hát
  ...
  try {
    ...
    let audio_path = null;
    let cover_path = null; // ★ đường dẫn ảnh bài hát
    ...
    // ★ Upload ảnh bài hát (tùy chọn) vào bucket covers
    if (coverFile) {
...
async function deleteChapter(ch) {
  ...
  const coverPaths = (tracks || []).map(t => t.cover_path).filter(Boolean); // ★ xóa cả ảnh bài hát
...
async function deleteTrack(track) {
  ...
  if (track.cover_path) { // ★ xóa cả ảnh bài hát

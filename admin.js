...
async function loadAdmin() {
  ...
  chapters = ch || [];
  renderChapters();
  renderArcsAdmin(); // ★ vẽ arc SAU khi đã có chapters → số chương + chip chính xác
}
...
// ★ Render arc: tên + số chương + danh sách chương bên trong (chip) + thêm/xóa chương
function renderArcsAdmin() {
  ...
  arcs.forEach((a, idx) => {
    ...
    const chipsHtml = chs.length
      ...
      : `<p class="hint">Chưa có chương trong arc này. Bấm + Chương để thêm.</p>`;
    ...
    // Nút × trên từng chip → xóa hẳn chapter đó (kèm nhạc bên trong)
    div.querySelectorAll(".arc-chip-del").forEach((delBtn, i) => {
      delBtn.onclick = () => deleteChapter(chs[i]);
    });
...
/* =========================================================
   ADMIN.JS — DÁN THÊM VÀO CUỐI FILE (khối mới, không xóa gì)
   Thêm chapter trực tiếp vào arc
========================================================= */

async function addChapterToArc(arc) {
  if (!story) return msg("arcMessage", "Hãy Save story trước.", true);

  const numberInput = prompt(`Chapter number cho arc "${arc.name}":`);
  if (numberInput === null) return;
  const number = Number(numberInput.trim());
  if (!Number.isInteger(number) || number < 1) {
    alert("Chapter number phải là số nguyên từ 1.");
    return;
  }
  if (chapters.some(c => Number(c.chapter_number) === number)) {
    alert(`Chapter ${number} đã tồn tại.`);
    return;
  }

  const titleInput = prompt(`Tiêu đề chapter ${number}:`);
  ...
  const style = prompt("Tone / style (tùy chọn):", "") || "";
  ...
  msg("arcMessage", `Đã thêm chapter ${number} vào arc "${arc.name}".`);
  await loadAdmin();
}